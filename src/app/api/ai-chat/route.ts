...
const SYSTEM_PROMPT = `
You are VIAN — Vivek Hingu's Personal Portfolio AI Assistant, built using the Agno Agent framework and powered by Groq's llama-3.1-8b-instant model.

You are an expert AI assistant designed to represent Vivek Hingu and answer questions about his skills, experience, projects, education, hackathons, certifications, and contact information.
...
- Personality: Natural, concise, helpful, friendly, technically strong, context-aware, honest.
- Tone: Professional yet approachable.

==================================================
KNOWLEDGE BASE (VIVEK HINGU)
==================================================
- Principal: Vivek Hingu
- Role: AI & Machine Learning Engineer / Full-Stack Developer
- Location: Ahmedabad, Gujarat, India
- Education: Bachelor of Engineering (B.E.) in Information Technology, SAL College of Engineering (CGPA: 8.61 / 10), July 2023 – June 2027.
...
==================================================
If the user asks a question about Vivek that is not covered in your knowledge base, clearly state that you do not have that specific information instead of inventing or hallucinating facts.

==================================================
WEB SEARCH DIRECTIVE
==================================================
Use web search capabilities only when current external information (news, live stats, external articles) is genuinely needed. Do not perform web searches for basic questions about Vivek's portfolio.
`;

/**
 * Dynamic RAG & Knowledge Synthesizer
 */
function synthesizeDynamicAgentResponse(query: string): string {
  ...
  if (/^(hi|hello|hey|greetings|namaste|hola|good\s*(morning|afternoon|evening))[\s!.?]*$/i.test(qLower)) {
    return "Greetings! 👋 I am **VIAN** — Vivek Hingu's Personal Portfolio AI Assistant (powered by Agno Framework & Groq `llama-3.1-8b-instant`).\n\nHow can I assist you with Vivek's **AI/ML projects**, **technical stack**, **hackathons**, or **contact details** today?";
  }
  ...
  if (qLower.includes("who") || qLower.includes("about") || qLower.includes("bio") || qLower.includes("vian") || qLower.includes("vivek")) {
    sections.push(`🤖 **Profile Overview**: Vivek Hingu is an AI & Machine Learning Engineer based in Ahmedabad, India, specializing in Agentic AI, RAG architecture, and full-stack software development.\n\n- **Education**: B.E. in Information Technology, SAL College of Engineering (CGPA: 8.61 / 10).`);
  }
  ...
  if (matchingProjects.length > 0 || (qLower.includes("project") && !qLower.includes("skill"))) {
    ...
    const projList = targetProjs
      .map((p) => `• **${p.title}** (${p.technologies.join(", ")})\n  - *Overview*: ${p.description}\n  - *Repository*: [GitHub](${p.href})`)
      .join("\n\n");
    sections.push(`### 🛠️ Portfolio Projects\n\n${projList}`);
  }
  ...
  if (matchingAchievements.length > 0 || matchingHackathons.length > 0 || qLower.includes("hackathon") || qLower.includes("award") || qLower.includes("winner")) {
    ...
    sections.push(`### 🏆 Competition Honors & Achievements\n\n${achList}`);
  }
  ...
  return `I am **VIAN** — Vivek Hingu's Personal Portfolio AI Assistant.\n\nRegarding **"${query}"**: I don't have that specific information in Vivek's portfolio context. Feel free to ask me about Vivek's **projects**, **AI/ML skills**, **education**, **hackathons**, or **contact details**!`;
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!body || typeof body.message !== "string" || !body.message.trim()) {
      return NextResponse.json(
        { response: "🤖 **VIAN Agno Agent Online** | Powered by Groq `llama-3.1-8b-instant`. State your query." },
        { status: 400 }
      );
    }
    ...
    if (!fullResponse) {
      if (/^[0-9+\-*/().^%\s]+$/.test(message) && message.length > 1) {
        ...
        fullResponse = `🌐 **Agent Tool Execution (DuckDuckGo Search)**:\n\n${searchRes.data}\n\n${synthesizeDynamicAgentResponse(message)}`;
      } else {
    ...
    return NextResponse.json(
      { response: "🤖 **VIAN Agent Core Active** | State your query regarding Vivek Hingu's portfolio." },
      { status: 500 }
    );
  }
}
