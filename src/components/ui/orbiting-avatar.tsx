...
const TECH_LOGOS = [
  {
    name: "Python",
    color: "#3776AB",
    bg: "from-blue-500/30 to-yellow-500/30",
  ...
  },
  {
    name: "Scikit-Learn",
    color: "#F7931E",
    bg: "from-blue-500/30 to-amber-500/30",
    border: "border-blue-400/50",
    icon: (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    name: "Next.js",
    color: "#FFFFFF",
    bg: "from-zinc-500/30 to-white/30",
  ...
  },
  {
    name: "TypeScript",
    color: "#3178C6",
    bg: "from-blue-600/30 to-sky-400/30",
    border: "border-blue-400/50",
    icon: (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    name: "Docker",
    color: "#2496ED",
    bg: "from-cyan-600/30 to-blue-500/30",
    border: "border-cyan-400/50",
    icon: (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
...
export function OrbitingAvatar({ profilePic, profilePicHover }: OrbitingAvatarProps) {
  ...
  return (
    <div className="relative flex items-center justify-center w-80 h-80 sm:w-[420px] sm:h-[420px] my-2 select-none">
      {/* 3D Orbit Ring Backing Trace (Widened so logos sit OUTSIDE photo) */}
      <div className="absolute w-[280px] h-[140px] sm:w-[380px] sm:h-[190px] rounded-[100%] border border-cyan-500/30 dark:border-cyan-400/25 shadow-[0_0_35px_rgba(34,211,238,0.2)] rotate-[-12deg] pointer-events-none" />

      {/* Avatar Photo in Center */}
      <div
        className="group relative z-30 cursor-pointer transition-all duration-500 hover:scale-105"
        onClick={() => playTapSound("chime")}
      >
        {/* Outer Halo */}
        <div className="absolute -inset-2.5 rounded-full bg-gradient-to-tr from-cyan-500 via-sky-400 to-indigo-500 opacity-55 blur-lg group-hover:opacity-90 transition-opacity duration-500 animate-pulse" />
      ...
      {/* 3D Orbiting Circular Tech Badges (Wide Radius so zero face overlap!) */}
      {TECH_LOGOS.map((tech, i) => {
        const itemAngle = angle + (i * Math.PI * 2) / TECH_LOGOS.length;

        // Widened 3D Elliptic Spatial Radius
        const rx = 165; // Horizontal radius (Out of face bounds)
        const ry = 80; // Vertical tilt radius (Below chin & above head)
        ...
        const zIndex = z > 0 ? 50 : 10; // Front logos zIndex 50 (in front), Back logos zIndex 10 (behind avatar)
        const scale = 0.8 + (z + 1) * 0.2; // Scale 0.8 to 1.2
        const opacity = 0.55 + (z + 1) * 0.225; // Depth opacity
        ...
        return (
          <div
            ...
            className="absolute group flex items-center justify-center cursor-pointer transition-transform duration-75"
            onClick={() => playTapSound("pop")}
          >
            {/* Sleek 3D Circular Badge */}
            <div
              className={`relative p-2.5 sm:p-3 rounded-full bg-black/85 backdrop-blur-2xl border ${tech.border} bg-gradient-to-br ${tech.bg} shadow-2xl transition-all duration-300 group-hover:scale-125 group-hover:border-cyan-300 group-hover:shadow-[0_0_25px_rgba(34,211,238,0.5)]`}
              style={{ color: tech.color }}
            >
              {tech.icon}

              {/* Tooltip Label on Hover */}
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none whitespace-nowrap px-2.5 py-0.5 rounded-full bg-black/90 text-[10px] font-bold text-white border border-white/20 backdrop-blur-md shadow-lg">
                {tech.name}
              </div>
            </div>
          </div>
